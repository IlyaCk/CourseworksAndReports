...
public class SupervisorController {
    ...
    public List<DisciplineDTO> getWorks(@AuthenticationPrincipal OAuth2User principal) {
        List<Discipline> disciplines = disciplineRepository.findBySupervisors_Email(principal.getName());
        return disciplines.stream()
                .map(discipline -> {
                    List<Work> filteredWorks = discipline.getWorks().stream()
                            .filter(work -> work.getSupervisor() != null && principal.getName().equals(work.getSupervisor().getEmail()))
                            .toList();
